...
module.exports = function(app,db,key){
  app.post('/computerparts/cases/:key',(req,res)=>{
    ...
    const OBJ={name:req.body.name,type:req.body.type,price:req.body.price,color:req.body.color,picture:req.body.picture}
  ...
  app.get('/computerparts/cases',(req,res)=>{
    db.collection('Case').find().toArray((err,item)=>{
      if(err){
        res.send({'error':'An error has occured'})
      }
      else{
